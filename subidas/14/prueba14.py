...
#------------------------------------------------------------------------------
# FUNCIÓN QUE CONSTRUYE EL MODELO DE SVM CON LOS MEJORES PARÁMETROS OBTENIDOS HASTA EL MOMENTO
def obtener_svm_reg():
    parameters = {
                    'kernel':['linear', 'poly', 'rbf', 'sigmoid'], #'poly', 'rbf', 'sigmoid' Specifies the kernel type to be used in the algorithm
                    'degree': range(0, 5), # Degree of the polynomial kernel function ('poly'). Must be non-negative. Ignored by all other kernels.
                    'gamma': ['scale', 'auto'], # 'auto' Kernel coefficient for 'rbf', 'poly' and 'sigmoid'.
                    'coef0': range(0, 5), # Independent term in kernel function. It is only significant in 'poly' and 'sigmoid'.
                    'tol': [0.001], # Tolerance for stopping criterion.
                    'C': [50.0, 100.0, 200.0, 250.0], # Regularization parameter. The strength of the regularization is inversely proportional to C. Must be strictly positive
                    'epsilon': [0.08, 0.09, 1.0, 1.1, 1.2], #  It specifies the epsilon-tube within which no penalty is associated in the training loss function with points predicted within a distance epsilon from the actual value
                    'shrinking': [True, False], # Whether to use the shrinking heuristic.
                 }
    #grid_search = GridSearchCV(SVR(), parameters)
    #print("Ajustando mejores parámetros para el modelo de regresión base...")
    ...
    #best_shrinking = grid_search.best_params_["shrinking"]
    #print("\n..... MEJORES PARÁMETROS:\n"+str(grid_search.best_params_))

    # Construir un modelo bagging de regresión con los mejores parámetros encontrados
    bagged_model = SVR(kernel='linear', degree=0, gamma='scale', coef0=0,
                    tol=0.001, C=50, epsilon=1.0, shrinking=True,
                    cache_size=200, max_iter=-1)
    bagging_model = BaggingRegressor(bagged_model, 100)
    return bagging_model

#------------------------------------------------------------------------------
# FUNCIÓN QUE CONSTRUYE EL MODELO DE LINEAR REGRESSOR CON LOS MEJORES PARÁMETROS OBTENIDOS HASTA EL MOMENTO
def obtener_linear_regressor():
    parameters = {
    ...
    #print("Ajustando mejores parámetros para el modelo LinearRegression base...")
    #grid_search.fit(X_train, y_train) # Entrenar conjunto de entrenamiento para encontrar mejores parámetros para el modelo base

    # Aislar la mejor combinación de parámetros
    #best_fit_intercept = grid_search.best_params_["fit_intercept"]
    #best_copy_X = grid_search.best_params_["copy_X"]
    #best_n_jobs = grid_search.best_params_["n_jobs"]
    #best_positive = grid_search.best_params_["positive"]
    #print("\n..... MEJORES PARÁMETROS:\n"+str(grid_search.best_params_))

    # Construir un modelo de regresión con los mejores parámetros encontrados
    bagged_model = LinearRegression(fit_intercept=True, copy_X=True,
...
def obtener_stacking():
    ...
    print("\nÁRBOL DE REGRESIÓN MONTADO.")
    nivel0.append(('knn', obtener_knn()))
    print("\nKNN MONTADO.")
    nivel0.append(('linearreg', obtener_linear_regressor()))
    print("\nMODELO DE REGRESIÓN LINEAL MONTADO.")
    nivel0.append(('svr', obtener_svm_reg()))
    print("\nSVR MONTADO.")
    print("\nMONTANDO STACKING...")
    stacking_model = StackingRegressor(estimators=nivel0)
    print("STACKING MONTADO.")
