...
# ## Aplico modelo
#
# Voy a aplicar un bagging de modelos KNN
from sklearn.neighbors import KNeighborsRegressor
from sklearn.model_selection import GridSearchCV
parameters = {
                "n_neighbors": range(1, 20), # Number of neighbors to use
                "weights": ["uniform", "distance"], # Weight function used in prediction
                "algorithm": ['auto', 'ball_tree', 'kd_tree', 'brute'], # Algorithm used to compute the nearest neighbors. 'auto' will attempt to decide the most appropriate algorithm based on the values passed to fit method
                "leaf_size": range(1, 10) # Leaf size passed to BallTree or KDTree. 30 by default
             }
grid_search = GridSearchCV(KNeighborsRegressor(), parameters)
print("Ajustando mejores parámetros para el modelo KNN base...")
grid_search.fit(X_train, y_train) # Entrenar conjunto de entrenamiento para encontrar mejores parámetros para el KNN base

# Aislar la mejor combinación de parámetros
best_k = grid_search.best_params_["n_neighbors"]
best_weights = grid_search.best_params_["weights"]
best_alg = grid_search.best_params_["algorithm"]
...
bagged_knn = KNeighborsRegressor(n_neighbors=best_k, weights=best_weights, algorithm=best_alg, leaf_size=best_leaf_size)
