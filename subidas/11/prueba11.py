...
parameters = {
                'min_child_weight':[3,4], # 4->3
                'gamma':[i/10.0 for i in range(0,3)], # 0.3->0.0
                'subsample':[i/10.0 for i in range(8,12)], # 0.9->0.9
                'colsample_bytree':[i/10.0 for i in range(8,12)], # 1.0->0.8
                'max_depth': [4,5,6], # 4->5
                'n_estimators':[10,50,100] # 50->100
             }
#grid_search = GridSearchCV(xgb.XGBRegressor(), parameters)
print("Ajustando mejores parámetros para el modelo XGBoost base...")
...
bagged_model = xgb.XGBRegressor(min_child_weight=3,
...
bagging_model = BaggingRegressor(bagged_model, n_estimators=200)
