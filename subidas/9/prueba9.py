...
parameters = {
                ...
                "min_samples_split": range(2, 10), # The minimum number of samples required to split an internal node
                "min_samples_leaf": range(1, 10), # The minimum number of samples required to be at a leaf node
                "max_features": [None, 80, "auto", "sqrt", "log2"] #  The number of features to consider when looking for the best split
                                    # 80 es el número total de columnas del dataset, exceptuando la clase
             }
grid_search = GridSearchCV(DecisionTreeRegressor(), parameters)
print("Ajustando mejores parámetros para el modelo de árbol de decisión de regresión base...")
...
bagged_model = DecisionTreeRegressor(criterion=best_criterion, min_samples_split=best_min_samples_split,
